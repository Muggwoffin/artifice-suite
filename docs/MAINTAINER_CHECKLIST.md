...
| **SQLite URI form on Windows** | `tropy_write.py` builds `file:{as_posix()}`; `tropy_db.py` builds `file:{db_path}`. Per the SQLite URI spec, `file:path` without `//` is relative to the CWD, so an absolute Windows path may need `file:///C:/…`. Cannot be settled from WSL — both forms behave identically there. Needs a run on native Windows against a real `.tropy`. **Do not apply a speculative fix**: `tropy_db.py` works today, and a blind change risks breaking a path that is fine. |
