...
def _send_request_with_retry(
    ...
    """Route the request to the correct provider with retry logic."""
    if config.llm_provider == LLMProvider.OLLAMA:
        return _send_ollama_request(model, sys_prompt, user_prompt, config)
    elif config.llm_provider == LLMProvider.OPENAI:
        return _send_openai_request(model, sys_prompt, user_prompt, config)
