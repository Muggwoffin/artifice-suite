...
4. **Local Testing**: Install the suite with `uv sync --extra all` (or `pip install -e apps/<app-name>` per app), run `pytest` inside each changed app, and `docker-compose build` locally before submitting pull requests.
