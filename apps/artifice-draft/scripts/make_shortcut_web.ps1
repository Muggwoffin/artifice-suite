...
.DESCRIPTION
    ...
    Interpreter choice prefers one that also has pywebview, since that is
    what gives a native window instead of opening your default browser; the
    web build still runs without it (add --browser, or let the launcher fall
    back on its own), same policy as the desktop build's tkinterdnd2
    preference.

    Re-run this after moving the project or reinstalling Python: the
    shortcut stores an absolute path to the interpreter, which the launcher
    cannot repair by itself once it is gone.
...
if (-not $chosen) {
    throw "No installed Python has the web dependencies. Run 'pip install -r requirements.txt -r requirements-web.txt' in $root first."
}
